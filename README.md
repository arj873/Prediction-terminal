...
plus live stock and crypto prices, [FRED](https://fred.stlouisfed.org) economic
data, the [Billboard](https://www.billboard.com/charts/) charts, and the
...
touches an account at any of the three exchanges — the terminal uses only
public, unauthenticated endpoints, and no price or entertainment feed needs a
key either. That is a deliberate limit, and it is visible: Polymarket US serves
its price history and prints only to an authenticated caller, so those two
panels say so rather than appearing broken. Scraped sources are third-party
sites whose markup can change without notice; the parsers are written to degrade
with a diagnosable error rather than silently return wrong numbers.
