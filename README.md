...
catalogue before it charts. predict.fun names a contract with two identifiers,
a slug for the question and a number for the leg, so a reference joins them with
...
| --- | --- | --- | --- | --- | --- | --- |
| Series | `KXFEDDECISION` | `fomc` | `usfed-fomc` | `FED` | `fed-rate` | `FF` |
| Event | `KXFEDDECISION-26OCT` | `fed-decision-in-october-2026…` | `usfed-fomc-2026-10-28` | `FED260917` | `what-will-fed-rate-hit-before-2027` | `FF_091726` |
| Rung | `Cut 25bps` | `25 bps decrease` | `25 bps Decrease` | `Maintain` | `↑ 5.5%` | `3.625` |
...
JSON arrays as JSON *strings*, Gemini quotes dollar strings and states its
24h move as a *percent*, predict.fun ships trade sizes and prices as 1e18-scaled
integer strings, and ForecastEx wraps every response in a JSON *string* inside a
JSON object — and none of that leaks past the server boundary. A normalised
market from any of the six is the same shape.
...
complement is its own: it rounds to the *market's* decimal precision, which is
not always the category's, and using the wrong one moves the NO side by a tick.
...
`Origin` header with a hard `403`, so the server sends none. GraphQL also
reports failure as an HTTP `200` with an `errors` array, so the client inspects
the body rather than the status, and treats a `null` entity with no error as the
`not_found` it is.
...
there is no bid, no ask and no ladder anywhere in its public API, and live
quotes sit behind IBKR ForecastTrader's login. `OB` on an `fx:` contract says
that rather than synthesising a one-level ladder out of the last two prints.
Its turnover, its daily move and its bars come from the exchange's
end-of-session CSV archive, which is published after the 16:30 CT close, so
those figures are a session behind the tape and the panel says so.

**A wrong sort key can hide a tenth of a catalogue.** Crawling ForecastEx by
open interest returns 11,466 rows and only 10,397 distinct contracts: ties are
not broken, so the offset window shifts under the crawl and 1,069 contracts are
served twice while as many are never served at all. Ordering by `contract_id`
returns each one exactly once. Gemini's tape has the same shape of problem for a
different reason — identical requests answer 278 or 349 prints depending on
which replica replies — so its turnover is read off its candles instead.
...
Kalshi uses, so every venue's bars land on one x-axis,
...
| `GET /api/venue/:venue/markets/:id/candles?interval=&start=&end=` | Candles (501 at Polymarket US, and at ForecastEx for `interval=1`) |
...
caller, and predict.fun's documented REST API gates even its read paths behind
an account key, so those panels name the endpoint that would answer and what it
