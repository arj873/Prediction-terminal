 ...
 * client, so a source that is added to one and not the others is a compile
 ...
 * folding one would 404 it. Each source declares which it is, so folding
 * happens here, once, and never at a call site.
 */

/**
 * What a source is *for*, which decides which command opens it.
 *
 * Declared rather than inferred from whether a method happens to be defined:
 * `ECOS` has to know which sources it may search before it asks any of them,
 * and a reader picking a prefix has to be told that `sec:` is not something
 * `ECO` will chart.
 */
...
/** The table's own row shape. `id` widens to `string` so `DataSourceId` can derive from it. */
...
const SOURCE_TABLE = [
  ...
  },
  {
    ...
    covers: 'CPI, unemployment, payrolls, earnings, productivity — the US labour statistics of record',
  ...
  },
  {
    ...
    covers: 'crude, gasoline, natural gas, electricity and generation — the feeds energy markets settle on',
    aliases: ['eia', 'energy'],
    key: {
      env: 'EIA_API_KEY',
  ...
  },
  {
    ...
    covers: 'consolidated US equity, index, FX and crypto bars — the paid tape behind STK, CRY and IMP',
...
export type DataSourceId = (typeof SOURCE_TABLE)[number]['id'];
...
export const DATA_SOURCE_IDS: readonly DataSourceId[] = DATA_SOURCES.map((s) => s.id);

/** The sources `ECO` can chart and `ECOS` can search, in table order. */
export const SERIES_SOURCE_IDS: readonly DataSourceId[] = DATA_SOURCES.filter(
  (s) => s.kind === 'series',
).map((s) => s.id);

const BY_ID = new Map<DataSourceId, DataSourceInfo>(DATA_SOURCES.map((s) => [s.id, s]));

export function dataSourceInfo(id: DataSourceId): DataSourceInfo {
  const info = BY_ID.get(id);
  if (!info) throw new Error(`Unknown data source "${id}"`);
  return info;
}

export function isDataSource(value: string): value is DataSourceId {
  return BY_ID.has(value as DataSourceId);
}

/** The source an unprefixed reference belongs to. */
export const DEFAULT_DATA_SOURCE: DataSourceId = (
  DATA_SOURCES.find((s) => s.isDefault) ?? DATA_SOURCES[0]!
).id;

/** Alias → source, built from the table rather than maintained beside it. */
const ALIASES: ReadonlyMap<string, DataSourceId> = new Map(
...
export function normaliseDataId(source: DataSourceId, id: string): string {
  const { case: casing } = dataSourceInfo(source);
 ...
 * about FRED.
 */
export function parseDataRef(raw: string, fallback: DataSourceId = DEFAULT_DATA_SOURCE): DataRef {
  const text = raw.trim();
