 ...
 *
 * Nothing here describes what a command *does* — that is the registry's job,
 * and saying it twice is how the two would come to disagree. A menu entry names
 * a command line and a label for the thing you are trying to accomplish; the
 * usage, the summary and the key that runs it are all looked up live. So a verb
 * renamed in the registry, or a key rebound with `KEYS`, changes what the menu
 * says without anyone editing this file.
 *
 ...
 * menu carries every command in the registry, so "everything is reachable from
 * the bar" is a property of the code rather than a promise someone has to keep
 * updating.
 */

import { ENT_GENRES } from '../../shared/types.js';
import { THEMES } from '../state.js';
import { formatChord, type Binding, type Keymap } from './keys.js';
import { COMMANDS, COMMAND_INDEX, type Command } from './registry.js';
...
function item(label: string, command: string, note?: string): MenuItem {
  return note === undefined ? { kind: 'item', label, command } : { kind: 'item', label, command, note };
}
...
/** The registry entry behind an entry's command, when it names one. */
...
/** Every command in one registry group, each invoked the way it prefers. */
function commandsIn(group: Command['group']): MenuEntry[] {
  return COMMANDS.filter((command) => command.group === group).map((command) =>
    item(command.summary, defaultInvocation(command)),
  );
}

/**
 * A verb's worked examples, straight from the registry.
 *
...
/** The verbs whose examples the LEARN menu offers. Checked against the registry. */
export const EXAMPLE_VERBS: readonly string[] = ['SRCH', 'GP', 'XV', 'IMP', 'STK', 'NEWS', 'FRED', 'BB'];
...
export const MENUS: readonly Menu[] = [
  ...
  },
  {
    ...
    entries: [
      ...
      group('Theme', THEMES.map((theme) => item(titleCase(theme), `THEME ${theme}`))),
      SEP,
      group('Panel focus', [
