...
mod tests {
    ...
    #[test]
    fn zz_differential_dump() {
        let Ok(corpus) = std::env::var("XV_CORPUS") else { return };
        let text = std::fs::read_to_string(&corpus).unwrap();
        let lines: Vec<&str> = text.lines().collect();
        let mut out: Vec<String> = Vec::new();
        let n = |v: &[f64]| v.iter().map(|x| format_number(*x)).collect::<Vec<_>>().join(",");
        for l in &lines {
            out.push(format!(
                "{}\t{}\t{}\t{}\t{}\t{}\t{}",
                serde_json::to_string(l).unwrap(),
                serde_json::to_string(&normalise(l)).unwrap(),
                tokenise(l).join("|"),
                content_tokens(l).join("|"),
                identity_key(l),
                n(&numbers_in(l)),
                n(&significant_numbers(l)),
            ));
        }
        let fmt = |m: &MatchScore| {
            format!("{:.15}\t{}\t{}\t{}", m.score, m.confidence.as_str(), m.shared.join("|"), m.reason)
        };
        for i in 0..lines.len() {
            for j in [(i + 1) % lines.len(), (i + 7) % lines.len(), (i + 31) % lines.len()] {
                let a = d(lines[i], lines[i]);
                let b = d(lines[j], lines[j]);
                out.push(format!("S\t{i}\t{j}\t{}", fmt(&score_series(&a, &b))));
                out.push(format!("E\t{i}\t{j}\t{}", fmt(&score_event(&a, &b))));
            }
        }
        for s in [0.0, 0.33999, 0.34, 0.4, 0.5999, 0.6, 0.7, 0.7999, 0.8, 0.95, 1.0] {
            out.push(format!("C\t{}\t{}", format_number(s), confidence_of(s).as_str()));
        }
        let mut i = 0usize;
        while i + 5 <= lines.len() {
            let left = &lines[i..i + 5];
            let start = (i + 60) % lines.len();
            let right = &lines[start..(start + 5).min(lines.len())];
            let pairs = pair_labels(left, right)
                .iter()
                .map(|p| format!("{}>{}@{:.15}", p.left, p.right, p.matched.score))
                .collect::<Vec<_>>()
                .join(" ");
            out.push(format!("P\t{i}\t{pairs}"));
            i += 5;
        }
        std::fs::write(std::env::var("XV_OUT").unwrap(), out.join("\n") + "\n").unwrap();
    }
