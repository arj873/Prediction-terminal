 ...
 * than a `=== 'kalshi'` somewhere downstream.
 *
 * This module is deliberately one of two halves. `crates/core/src/venue.rs` is
 * the other, and carries the same table and the same four functions: the server
 * needs them to route a request, and the client needs `parseRef` synchronously
 * at the prompt — before any request exists — so code generation cannot supply
 * it. What generation *can* pin is the identifiers, so `Venue` and `MoverSort`
 * are imported from the Rust bindings rather than derived from the table below:
 * a broker added to one half and not the other is a compile error here.
 * Behaviour is pinned by `contract/venue-cases.json`, a parity fixture both
 * halves' tests read, so a rule changed on one side fails on the other.
 */

import type { MoverSort, Venue } from '$gen';

/**
 * Re-exported so a caller that reads a reference does not also have to know
 * which generated file the venue identifiers live in.
 */
export type { MoverSort, Venue };
...
/** The table's own row shape. `id` is the generated `Venue`, so the table cannot name a broker the server does not have. */
